...
Beyond the scope of this thesis, motor imagery BCIs face a set of challenges that affect their deployment in real clinical and daily-use environments.
...
Standard mitigation strategies include bandpass filtering to suppress out-of-band noise (as applied in this thesis), independent component analysis (ICA) to separate artefact components from neural sources, and automated artefact rejection based on peak-to-peak amplitude thresholds~\cite{yadav2025decoding}. For clinical BCI deployment, online artefact detection is essential because the user cannot be asked to remain still or re-record contaminated trials.
...
A subset of users show little or no detectable ERD/ERS modulation during motor imagery. These non-responders — sometimes termed \textit{BCI-illiterate} — cannot achieve reliable control with standard MI paradigms regardless of classifier choice~\cite{lotte2018review}. In this thesis, two subjects were excluded after diagnostics confirmed atypical lateralisation patterns rather than recording or pipeline errors. Addressing non-response remains an open problem; proposed approaches include extended neurofeedback training, alternative paradigms such as SSVEP, and personalised adaptive models.
