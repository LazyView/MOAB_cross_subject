"""
Cross-subject evaluation using ShallowConvNet (braindecode) with MOABB CrossSubjectEvaluation.
...
Results saved to results/shallow_cross_subject_eval[_excluded_outliers].csv.
Run from project root: python evaluation/shallow_cross_subject_eval.py
"""
...
print(f"ShallowConvNet input: {n_chans} channels × {n_times} samples")
...
print("\n=== ShallowConvNet Cross-Subject Evaluation Results ===")
print(pivot.to_string())

scores = results[results['pipeline'] == 'ShallowConvNet']['score']
print(f"\n  ShallowConvNet  {scores.mean():.3f} ± {scores.std():.3f}")
print(f"  Chance level: 0.500")

# --- Save results ---
suffix = "_excluded_outliers" if EXCLUDE_OUTLIERS else ""
out_path = RESULTS_DIR / f"shallow_cross_subject_eval{suffix}.csv"
